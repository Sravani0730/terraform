...
resource "aws_key_pair" "example" {
  ...
  public_key = file("C:/Users/srava/.ssh/id_rsa.pub")
}

# IAM Policy for S3 access
resource "aws_iam_policy" "s3_access_policy" {
  name        = "EC2S3AccessPolicy123"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = [
          "arn:aws:s3:::terraforms3practice",
          "arn:aws:s3:::terraforms3practice/*"
        ]
      }
    ]
...
resource "aws_instance" "web_server" {
  ami                  = "ami-0d682f26195e9ec0f" # Update with a valid AMI ID
...
resource "null_resource" "setup_and_upload" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "aws s3 cp /var/www/html/index.html s3://terraforms3practice/",
  ...
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = file("C:/Users/srava/.ssh/id_rsa")
