...
provider "aws" {
 ...
 resource "aws_s3_bucket" "dev" {
    bucket = "statefile-configuration123"
    
  
}

#after run this will get error like The variable 'aws_region' must be one of the following regions: ap-south-2,│ eap-south-1, so it will allow any one region defined above in conditin block
